...
class QattenMixer(nn.Module):
    def __init__(self, args):
        ...
        self.state_mlp = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(self.mlp_hidden_dim, self.query_dim)
        )
        self.state_gru = nn.GRUCell(self.query_dim, self.query_dim)
        self.state_gru_hidden = None
        self.W_Q = nn.Linear(self.query_dim, self.query_dim)
        self.W_K = nn.Linear(self.role_embedding_dim, self.key_dim)
        self.scaled_product_value = np.sqrt(self.query_dim)
    ...
    def forward(self, agent_qs, states, role_embeddings):
        bs_len = agent_qs.size(0)
        N = self.N
        # states: (bs_len, state_dim)
        state_embed = self.state_mlp(states)
        if self.state_gru_hidden is None or self.state_gru_hidden.shape[0] != bs_len:
            self.state_gru_hidden = torch.zeros(bs_len, self.query_dim, device=states.device)
        state_embed = self.state_gru(state_embed, self.state_gru_hidden)
        self.state_gru_hidden = state_embed.detach()
        Q = self.W_Q(state_embed)  # (bs_len, query_dim)
        Q = Q.unsqueeze(1)         # (bs_len, 1, query_dim)
        K = self.W_K(role_embeddings)  # (bs_len, N, key_dim)
        K = K.permute(0,2,1)       # (bs_len, key_dim, N)
        attn_logits = torch.matmul(Q, K) / self.scaled_product_value  # (bs_len, 1, N)
        attn_weights = torch.softmax(attn_logits, dim=-1)             # (bs_len, 1, N)
        agent_qs = agent_qs.unsqueeze(1)                              # (bs_len, 1, N)
        q_total = torch.sum(agent_qs * attn_weights, dim=-1)          # (bs_len, 1)
        c = self.constrant_value_layer(states)                        # (bs_len, 1)
        q_total = q_total + c                                         # (bs_len, 1)
        return q_total
...
class BAVDM_Agent(object):
    ...
    def update_qmix(self, inputs, batch_o, batch_s, batch_r, batch_a, batch_last_a, batch_avail_a_n, batch_active, batch_dw, max_episode_len):
        ...
        _, role_embeddings = self.RECL.batch_role_embed_forward(batch_o, batch_last_a, max_episode_len, detach=False) # (batch_size, max_episode_len+1, N, role_embed_dim)
        ...
        # role embed
        role_embeddings_eval = role_embeddings[:, :-1].reshape(-1, self.N, self.role_embedding_dim)
        role_embeddings_target = role_embeddings[:, 1:].reshape(-1, self.N, self.role_embedding_dim)
        batch_s_eval = batch_s[:, :-1].reshape(-1, self.state_dim)
        batch_s_target = batch_s[:, 1:].reshape(-1, self.state_dim)
        q_evals_flat = q_evals.reshape(-1, self.N)
        q_targets_flat = q_targets.reshape(-1, self.N)

        # calculate the globle Q value
        q_total_eval = self.eval_mix_net(q_evals_flat, batch_s_eval, role_embeddings_eval)
        q_total_target = self.target_mix_net(q_targets_flat, batch_s_target, role_embeddings_target)

        q_total_eval = q_total_eval.view(self.batch_size, max_episode_len, 1)
        q_total_target = q_total_target.view(self.batch_size, max_episode_len, 1)
